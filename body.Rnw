...
\begin{figure}[!ht]
  \centering
	\includegraphics[width=.5\textwidth]{modified-figs/method.png}
...
Our ability to confidently extrapolate carbon density to unmeasured depth is important in order to predict inland soil carbon stock both for conservation and carbon budgeting as well as for ecological comparison. 
...
The hierarchical model employed here will be useful because 

We did not extrapolate to depth. But this model could be used to and extended in order to do so. Our key result was that the shape of the decline varied by wetland, and so it is important to 

\subsection*{Landscape drivers of soil carbon stock}

Carbon storage in wetlands is driven by a wide range of interacting factors. Carbon was highest among wetlands with intermediate levels of precipitation but in areas with infrequent permanent water cover. I also found that sites with both low and high productivity, as approximated by NDVI, had higher carbon. Possibly, these results indicate that there are a few distinct plant communities, on both extremes of the NDVI spectrum, that exist in wetlands with high carbon stock \citep{whitaker2015,wang2016,marin-muniz2014,ceballos2013}.
